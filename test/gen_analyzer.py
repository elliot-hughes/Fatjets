...
Rs = [0.8, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0]
...
h1 = {#	name			x-axis label		bins	xmin	xmax	W		S		E		N
	"njets":	["", "Number Of Fatjets",		60,		0,		60,		0.7,	0.65,	0.85,	0.85],
...
for R in Rs:
	print R
	tf_in = TFile("fatjets_sgtosq_msg{0}_msq{1}_R{2}{3}.root".format(msg, msq, int(R*10), output_string))	# Open the TFile
	tt_in = {}
...
#	tl.SetHeader("The Legend Title")
	...
	tl.Draw()
	if save_plots:
		c1.SaveAs("plots/h1_{0}_{1}_msg{2}_msq{3}{4}.pdf".format(key, "fat_gen", msg, msq, output_string))
		c1.SaveAs("plots/h1_{0}_{1}_msg{2}_msq{3}{4}.svg".format(key, "fat_gen", msg, msq, output_string))
gPad.SetBottomMargin(0.15)
...
#	tl.Draw()
	if save_plots:
		c1.SaveAs("plots/h2_{0}_{1}_msg{2}_msq{3}{4}.pdf".format(key, "fat_gen", msg, msq, output_string))
		c1.SaveAs("plots/h2_{0}_{1}_msg{2}_msq{3}{4}.svg".format(key, "fat_gen", msg, msq, output_string))
